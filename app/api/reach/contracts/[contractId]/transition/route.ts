 ...
 *
 * Auth required: caller must be a party to the contract.
 ...
 *   - Initiator → INITIATOR event actor
 *   - Target → TARGET event actor
 */
...
export async function POST(
  ...
  // Verify contract exists and caller is a party.
  const contract = await getContract(contractId);
  if (!contract) {
    return Response.json({ ok: false, error: 'Contract not found' }, { status: 404 });
  }

  let eventActor: ReachContractEventActor;
  if (contract.initiatorId === auth.actorId) {
    eventActor = 'INITIATOR';
  } else if (contract.targetId === auth.actorId) {
    eventActor = 'TARGET';
  } else {
